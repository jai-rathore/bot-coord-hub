...
export function getDiscoveryDocument(baseUrl?: string) {
  ...
  return {
    ...
    recruiting_agent_instructions:
      "When a recruiter has a job URL or description, call draft_hiring_role first. Show the extracted terms, resolve locationQueries with resolve_discovery_location, and wait for explicit approval before using suggestedPrivateConfig. When a candidate shares a /:handle?hire=1 link, call get_agent_profile and confirm recruiting.acceptsRoleBriefs, then call propose_hiring_role with those approved terms; never invent compensation, equity, location, or other terms. The candidate agent returns only human-approved gaps or expectations. After mutual introduction approval, request_schedule_meeting may coordinate a call but still pauses at human confirmation.",
    skill: {
